...
## Phase 3 — Bank Connection (Monzo) ✅ Complete — pivoted from TrueLayer
Goal: User can connect their bank and real transactions flow into the app.

**Verified working end-to-end on 2026-09-04** — real Monzo account connected, 26 real transactions synced, all auto-categorised correctly (including the McDonald's → Eating Out case that originally motivated the few-shot examples below).
...
- ✅ Registered SaveMyMoney as a confidential OAuth client in Monzo's Developer Tools
- ✅ `connect-bank.tsx`: multi-bank picker grid replaced with a single "connect Monzo" card (the picker made no functional sense once only one provider actually worked — see comment in the file). Builds `https://auth.monzo.com/?client_id=...&redirect_uri=...&response_type=code&state=...` — **redirect_uri is a GitHub Pages URL, not `cait://auth/callback` directly** (see the redirect_uri saga below for why), and is sent as a plain unencoded string, not `encodeURIComponent()`'d — matches what actually worked
- ✅ `exchange-token`: points at `https://api.monzo.com/oauth2/token`, reads `MONZO_CLIENT_ID`/`MONZO_CLIENT_SECRET`; also fixed a pre-existing bug where `token_expires_at` was hardcoded to a fixed date instead of computed from `expires_in`
- ✅ `fetch-transactions`: swapped to `https://api.monzo.com/accounts` and `https://api.monzo.com/transactions?account_id=...&expand[]=merchant` (the `expand[]=merchant` param is required — without it `merchant` is a bare `merch_xxxx` id, not a name); removed the `Math.round(tx.amount * 100)` conversion since Monzo's `amount` is already signed pence, matching what `transactions.tsx`'s `formatAmount()` already expects — no sign flip needed either
- ✅ Renamed `truelayer_transaction_id` → `provider_transaction_id` via migration `20260904000000` (`RENAME COLUMN` + `RENAME CONSTRAINT`, both metadata-only, no data rewrite); `fetch-transactions` updated to match. Left `webhook-truelayer/index.ts`'s reference to the old column name alone since that function is dead/unregistered (see retirement task below) rather than editing code that isn't running
- ✅ Added `bank_connections_user_id_unique` constraint (migration `20260904000001`) and switched `exchange-token`'s insert to an upsert on `user_id` — the old plain `.insert()` would have created a second row on any re-connect, which `fetch-transactions`' `.single()` can't handle
- ⬜ Handle Monzo's 5-minute full-history window: the first sync after connecting must pull full transaction history immediately, since after 5 minutes the API restricts to the last 90 days only. Deferred — not blocking real use, 90 days is enough history for budgeting/categorisation right now
- ⬜ Retire `webhook-truelayer` (was never registered anyway) and the now-dead `supabase/functions/oauth-redirect` (see below — replaced by a GitHub Pages page, but left deployed rather than deleted mid-debugging)

#### The redirect_uri saga — three unrelated infra bugs stacked on top of each other
Getting the OAuth redirect working was the hardest part of this whole pivot, for reasons that had nothing to do with our own code being wrong:
1. **`cait://auth/callback` (a custom URL scheme) broke Monzo's login email.** Monzo requires `redirect_uri` to be `http(s)`, and its backend (Go) refuses to embed a non-http(s) URL into an HTML email — it silently substitutes the literal string `"ZgotmplZ"` instead of a real link, so the "log in" button in the magic-link email did nothing. Fix: register a real `https://` URL as `redirect_uri`, and have *that* page hand off to `cait://auth/callback` from inside the browser instead (a normal, unrestricted browser navigation, not an email template).
2. **Supabase Edge Functions/Storage can't serve real HTML on the free tier.** Built that `https://` landing page as a Supabase Edge Function first — it force-rewrites any HTML response to `text/plain` (deliberately, to stop arbitrary rendering pages being hosted under the trusted `supabase.co` domain), so the browser just showed the page's source code instead of running it. Fix: moved the same static page to a small dedicated public GitHub repo (`github.com/SalauDml/cait-oauth-redirect`) served via GitHub Pages, which has no such restriction.
3. **Monzo's developer console didn't reliably apply an edited `redirect_uri`.** Even with a correct, exactly-matching `https://` URL, token exchange kept failing with `bad_authorization_code.redirect_uri_mismatch` — a documented, repeatedly-reported Monzo issue where editing an *existing* client's settings doesn't reliably propagate server-side. Fix: created a brand-new OAuth client with the same settings instead of continuing to edit the old one, which worked immediately.

**Concepts covered:** Monzo's confidential-client OAuth flow, the ZgotmplZ Go template safe-URL sentinel, platform-level content-type restrictions on serverless functions, OAuth redirect_uri exact-match semantics (bound to the issued code, not just the client registration), XSS via `<script>`-tag breakout and why HTML-attribute escaping is safer than `JSON.stringify` into a script block, `verify_jwt = false` for public-by-design Edge Functions
